...
(function() {
  ...
  QuasiABS.Manager.startAction = function(self, targets, item) {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      if(!targets[i].battler().states().contains($dataStates[11]))
  ...
  Skill_Sequencer.prototype.update = function() {
    ...
    if (sequence) {
      ...
      this.startAction(action, targets);
    }
  ...
  Skill_Sequencer.prototype.startAction = function(action, targets) {
    switch (action[0].toLowerCase()) {
      ...
      case "targetjumphere":
    	  this.startTargetJumpHere(action, targets);
        break;
      }
  };

  Skill_Sequencer.prototype.startTargetJumpHere = function(action, targets) {
	    ...
	    for (var i = 0; i < targets.length; i++) {
	      ...
	      var x1 = targets[i]._px;
		  var y1 = targets[i]._py;
		  var x2 = this._skill.collider.center.x;
		  var y2 = this._skill.collider.center.y;
		  var dx = x2 - x1;
		  var dy = y2 - y1;
		  var final = this.adjustPosition(targets[i]._px, targets[i]._py, targets[i]._px + dx, targets[i]._py + dy);
		  if (!dist)
			  targets[i].pixelJump(final.x - targets[i]._px, final.y - targets[i]._py);
		  else {
			  alert("gg");
			  targets[i].pixelJump(final.x - targets[i]._px + 48, final.y - targets[i]._py);
		  }
		  //else targets[i].pixelJump(final.x - targets[i]._px - 48, final.y - targets[i]._py);
	      //else targets[i].pixelJump(final.x - targets[i]._px, final.y - targets[i]._py + 48);
	    ...
	    if (Input.isTriggered('#q')) {
	    	...
	    	//if(!$gameActors.actor(1).isDead())
		     ...
		     // $gamePlayer.requestAnimation(1);
		      ...
		      //this._actors[1] = this._actors[2];
		      //this._actors[2] = this._actors[3];
		      //this._actors[3] = temp;
	    	$gameParty.swapOrder(2,0);  
	    	$gameParty.swapOrder(1,0);
	    	while (this.actor().isDead()) {
  ...
  Game_Event.prototype.updateAI = function() {
    ...
    } else if (this.canMove() && $gameSelfSwitches.value(key) !== true) {
