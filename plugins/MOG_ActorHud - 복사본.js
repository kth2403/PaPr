 ...
 * (v1.2) - Correção do setup da posição do numero de tp. 
...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_ActorHud');
	...
	Moghunter.ahud_tp_meter_pos_y = Number(Moghunter.parameters['tp Meter Y-Axis'] || 85);
	Moghunter.ahud_tp_meter_rotation = Number(Moghunter.parameters['tp Meter Angle'] || 0);
	Moghunter.ahud_tp_meter_flow = String(Moghunter.parameters['tp Meter Flow Anime'] || true);
	...
	Moghunter.ahud_tp_number_pos_y  = Number(Moghunter.parameters['tp Number Y-Axis'] || 70);
	...
	Moghunter.ahud_maxtp_number_pos_y  = Number(Moghunter.parameters['Maxtp Number Y-Axis'] || 40);	
...
Actor_Hud.prototype.load_img = function() {
	...
	if (String(Moghunter.ahud_maxtp_number_visible) == "true") {
		this._maxtp_number_img = ImageManager.loadAHud("tp_Number2");
		this._tpSlash_img = ImageManager.loadAHud("tp_Number3");
		}
	if (String(Moghunter.ahud_maxmp_number_visible) == "true") {
		this._maxmp_number_img = ImageManager.loadAHud("MP_Number2");
		this._mpSlash_img = ImageManager.loadAHud("MP_Number3");
	}
};
...
Actor_Hud.prototype.update_face_animation = function() {
	if (this._battler._ahud_face_data[3] > 0) {this._battler._ahud_face_data[3] -= 1;
	    if (this._battler._ahud_face_data[3] === 0) {
			...
			else if (this._battler.tp <= 30 * 100 / 100) {this._battler._ahud_face_data[2] = 3}
...
Actor_Hud.prototype.create_maxmp_number = function() {
	...
	this._mpSlash = new Sprite(this._mpSlash_img);
	this._mpSlash.x = this._pos_x + Moghunter.ahud_maxmp_number_pos_x - 24
	this._mpSlash.y = this._pos_y + Moghunter.ahud_mp_number_pos_y + 1
	this.addChild(this._mpSlash);
