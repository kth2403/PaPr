...
(function () {
    ...
    QABSManager.startAction = function (self, targets, item) {
        ...
        for (var i = 0; i < targets.length; i++) {
            ...
            if (!targets[i].battler().states().contains($dataStates[11]) && !targets[i]._invincible && !targets[i]._avoid) {
...
(function () {
    ...
    Skill_Sequencer.prototype.startOnDamageTargetAction = function (action, targets) {
        ...
        switch (cmd) {
            ...
            case 'jumpZ': {
                this.targetJumpZ(action, targets, height);
                break;
            }
            case 'pose': {
    ...
    Skill_Sequencer.prototype.particleOn = function (action) {
        ...
        //this._skill.particle = action[0];
        this._skill.particle = [SceneManager.particleGens.length, action[1], action[2], action[3]];
        ...
        if (isAnimated) {
            ...
            var layers = Math.floor(Number(frames / 5)) || 1;
    ...
    Skill_Sequencer.prototype.userInvincible = function (action) {
        this._character._invincible = action[0] === "on" ? true : false;
    };
    ...
    Skill_Sequencer.prototype.skillDash = function (action) {
        ...
        var dist = Number(action[0]) || this._character.moveTiles();
        var route = {
            list: [{ code: 45 }, { code: 0 }],
            repeat: false,
            skippable: true,
    ...
    Skill_Sequencer.prototype.userForceSkill = function (action) {
        ...
        var id = Number(action[0]);
        var angleOffset = Number(action[1]) || 0;
        ...
        switch (mode) {
            ...
            case 3: //아직 사용하지 않음
                ...
                var independent = {
                    ...
                    radian: this._character._radian
                }
    ...
    Skill_Sequencer.prototype.targetJumpZ = function (action, targets, height) {
        ...
        for (var i = 0; i < targets.length; i++) {
            ...
            if (!targets[i].battler().states().contains($dataStates[11], $dataStates[12], $dataStates[17]) && !targets[i]._invincible && !targets[i]._avoid && !targets[i]._noKnock) {
                ...
                var prevRadian = targets[i]._radian;
                targets[i].setDirectionFix(true);
                targets[i].pixelZJump(dx, dy, height);
                targets[i].setDirectionFix(lastDirectionFix);
                targets[i].setRadian(prevRadian);
            }
        }
    ...
    Skill_Sequencer.prototype.canSkillMove = function () {
        ...
        if (targets.length > 0) {
            for (var i = targets.length - 1; i >= 0; i--) {
                if (!this._skill.targetsHit.contains(targets[i].charaId()) && !targets[i]._invincible && !targets[i]._avoid) {
                    this._skill.targetsHit.push(targets[i].charaId());
                } else {
                    if (targets[i]._avoid) {
                        if (!targets[i].battler().states().contains($dataStates[15])) {
                            targets[i].battler().addState(15);
                        }
                    }
                    targets.splice(i, 1);
                }
        ...
        if (through === 1 || through === 3) {
            ColliderManager.getCharactersNear(this._skill.collider, function (chara) {
                ...
                if (chara.isLoot || chara._erased || chara.isDead || chara._avoid) return false;
    ...
    Skill_Sequencer.prototype.onEnd = function () {
        ...
        this.particleOff(this.particle);
    ...
    Skill_Sequencer.prototype.updateSkillPosition = function () {
        ...
        if (skillRad > 2 * Math.PI || skillRad < 0)
        ...
        if (this._skill.guidD > 0) {
            ...
            //if (skillRad > toRadian + this._skill.guidSpeed/2) this._skill.radian -= this._skill.guidSpeed;
            //else if (skillRad < toRadian - this._skill.guidSpeed/2) this._skill.radian += this._skill.guidSpeed;
            this._skill.speedX = this._skill.speed * Math.cos(this._skill.radian);
            this._skill.speedY = this._skill.speed * Math.sin(this._skill.radian);
            this.setSkillPictureRadian(this._skill.picture, this._skill.radian);
        ...
        if (this._skill.picture) {
            this._skill.picture.move(collider.center.x, collider.center.y);
        }
        this._skill.duration--;
        if (!this.canSkillMove() || !this._skill.duration) {
...
(function () {
    ...
    Game_Enemy.prototype.setup = function (enemyId, x, y) {
        ...
        this._noKnock = meta.noKnock;///<noknock>/i.test(notes);
...
(function () {
    ...
    Game_CharacterBase.prototype.makeSkillCollider = function (settings, independent) {
        ...
        var x1 = independent ? independent.x - collider.center.x: this.cx() - collider.center.x;
        var y1 = independent ? independent.y - collider.center.y: this.cy() - collider.center.y;
        if (infront) {
    ...
    Game_Player.prototype.updateABSInput = function () {
        ...
        for (var key in absKeys) {
            ...
            for (var i = 0; i < inputs.length; i++) {
                ...
                if (input === 'mouse1' && (TouchInput.isTriggered() || TouchInput.isPressed()) && !Input.isPressed('control') && this.canClick()) {
                    //TouchInput.stopPropagation();
                    if (Input.isPressed('shift')) {
                        if ($gamePlayer._moveSpeed > 3.4 && $gamePlayer._isMoving)
                            this.useSkill(absKeys[11].skillId);
                        else this.useSkill(absKeys[12].skillId);
                    } else this.useSkill(absKeys[key].skillId);
                    break;
                }
                if (input === 'mouse2' && TouchInput.isCancelled() && !Input.isPressed('control') && this.canClick()) {
                    //TouchInput.stopPropagation();
                    this.useSkill(absKeys[key].skillId);
                    break;
                }
        ...
        if (!meta.dontTurn) {
            if (isGamepad && QABS.towardsAnalog) {
                ...
                if (horz !== 0 || vert !== 0) {
                    ...
                    /*var x1 = $gameMap.event(1).cx();
                    var y1 = $gameMap.event(1).cy();
                    var x2 = this.cx();
                    var y2 = this.cy();
                    skill.radian = Math.atan2(y1 - y2, x1 - x2);*/
                    skill.radian = independent ? independent.radian : this._radian;
                }
    ...
    Game_Event.prototype.setupLoot = function () {
        this.battler().makeDropItems().forEach(function (item) {
            if (DataManager.isWeapon(item)) {
                item.type = 1;
                $gameMap.copyItemFrom(1, 161, item, this.x, this.y, true);
            }
            else if (DataManager.isArmor(item)) {
                item.type = 2;
                $gameMap.copyItemFrom(1, 162, item, this.x, this.y, true);
            }
            else {
            item.type = 0;
                $gameMap.copyItemFrom(1, 160, item, this.x, this.y, true);
            }
        }, this);
        /*
        var x, y;
