...
(function() {
  ...
  QuasiABS.getSkillSettings = function(skill) {
    if (!this._skillSettings.hasOwnProperty(skill.id)) {
      ...
      if (settings) {
        ...
        this._skillSettings[skill.id].jumping = Number(settings.jumping) || 0;
        if(settings.groundtarget) this._skillSettings[skill.id].groundimage = settings.groundimage;
      }
    }
  ...
  //변경 : 만약 상태 있을경우 무적, 만약 jumping가 1일경우 회피가능
  QuasiABS.Manager.startAction = function(self, targets, item) {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      if(item.settings.jumping + targets[i].battler().states().contains($dataStates[12]) < 2 && !targets[i].battler().states().contains($dataStates[11]) && !targets[i]._invincible && !targets[i]._avoid)
...
//변경 : tXlight quasi로 오버라이딩 됨 , 글로벌락일때 스킬 안움직임 
  Skill_Sequencer.prototype.update = function() {
    ...
    if(!$gamePlayer._globalLocked || this._skill.mainCasting)
  ...
  Skill_Sequencer.prototype.updateSkillPosition = function() {
    ...
    x1 = x1 + this._skill.speedX;
    y1 = y1 + this._skill.speedY;
    this._skill.collider.moveto(x1, y1);
    
    if(this._skill.accelD || this._skill.rotateD || this._skill.guidD){
    	if(this._skill.accelD) {
    	...
    	if(this._skill.guidD) {
    		var x3 = $gamePlayer.cx();
    	    var y3 = $gamePlayer.cy();
    	    var x4 = this._skill.collider._center.x;
    	    var y4 = this._skill.collider._center.y;
    	    var toRadian = Math.atan2(-(y3 - y4), x3 - x4);
    	    toRadian += toRadian < 0 ? 2 * Math.PI : 0;
    	    var sat = Math.sin(this._skill.radian - toRadian);
    	    var sat2 = Math.cos(this._skill.radian - toRadian);
    	    //sat += sat < 0 ? 2 * Math.PI : 0;
 	       if(sat > 0 && sat2 < 0.9995) {
 	    	   this._skill.radian -= this._skill.guidSpeed;
 	       } else if(sat <= 0 && sat2 < 0.9995) {
 	    	  this._skill.radian += this._skill.guidSpeed;
 	       }
 	      this._skill.speedX = this._skill.speed * Math.cos(this._skill.radian);
 	     this._skill.speedY = this._skill.speed * -Math.sin(this._skill.radian);
 	    this.setSkillPictureRadian(this._skill.picture, this._skill.radian);
 	       ...
 	       this._skill.guidD --;
 	       //console.log(this._skill.guidSpeed, this._skill.guidA, this._skill.guidD)
    	}
	    /*this._skill.speed -= 0.1
	    if(this._skill.speed) {
	    	this._skill.speedX = this._skill.speed * Math.cos(this._skill.radian);
	    	this._skill.speedY = this._skill.speed * -Math.sin(this._skill.radian);
	    }
	    if(x1 < $gamePlayer.px) {
	    	this._skill.speedX += 0.1;
	    } else this._skill.speedX -= 0.1;
	    if(y1 < $gamePlayer.py ) {
	    	this._skill.speedY += 0.1;
	    } else this._skill.speedY -= 0.1;
*/
...
//변경 : globalLock에 메인캐스팅 변수 추가, 텔레포트 추가, 지진 추가
  Skill_Sequencer.prototype.startAction = function(action) {
    switch (action[0].toLowerCase()) {
      ...
      case "se":
        ...
        break;
      case "globallock":
        if (Imported.Quasi_MovementPlus) {
        	$gameMap.globalLock();
        	this._skill.mainCasting = true;
        }
        break;
      case "globalunlock":
        if (Imported.Quasi_MovementPlus) {
        	$gameMap.globalUnlock();
        	this._skill.mainCasting = false;
        }
        break;
      case "changeactor1":
    	  this.startChangeActor1(action);
          break;
      case "changeactor2":
    	  this.startChangeActor2(action);
          break;
      case "changeactor3":
    	  this.startChangeActor3(action);
          break;
      case "shake":
  ...
  Skill_Sequencer.prototype.startUserAction = function(action) {
    switch (action[1].toLowerCase()) {
      ...
      case "posemove":
    	  ...
    	  break;
      case "clearpose":
    	  this.userClearPose(action);
    	  break;
      case "forceskill":
        this.userForceSkill(action);
        break;
      case "forceskill2":
          this.userForceSkill2(action);
          break;
      case "forceskill3":
    	  this.userForceSkill3(action);
    	  break;
      case "animation":
  ...
  Skill_Sequencer.prototype.startDamageTargetAction = function(action, targets) {
    switch (action[1].toLowerCase()) {
      ...
      case "cancel":
        ...
        break;
      case "star":
          this.targetStar(action, targets);
          break;
  ...
  var cooldownsFor = {};
  var changeActor;
  Skill_Sequencer.prototype.startChangeActor1 = function(action) {
	  //if($gameParty.leader().actorId() === 1)
	  switch($gameParty.leader().actorId()){
	  case 1 :
		  cooldownsFor[0] = $gamePlayer._skillCooldowns;
		  break;
	  case 2 :
		  cooldownsFor[1] = $gamePlayer._skillCooldowns;
		  break;
	  case 3 :
		  cooldownsFor[2] = $gamePlayer._skillCooldowns;
		  break;
	  }
	  $gameParty.swapOrder(1,0);  
	  $gameParty.swapOrder(2,0);
	  while ($gamePlayer.actor().isDead()) {
	  $gameParty.swapOrder(1,0);
	  $gameParty.swapOrder(2,0);
  ...
  Skill_Sequencer.prototype.startChangeActor2 = function(action) {
	  switch($gameParty.leader().actorId()){
	  case 1 :
		  cooldownsFor[0] = $gamePlayer._skillCooldowns;
		  break;
  ...
  Skill_Sequencer.prototype.startShake = function(action) {
	  var power = Number(action[1]);
	  var speed = Number(action[2]);
	  var duration = Number(action[3]);
	  $gameScreen.startShake(power,speed,duration);
  };
  
  Skill_Sequencer.prototype.startShake = function(action, target) {
	  var power = Number(action[1]);
	  var speed = Number(action[2]);
	  var duration = Number(action[3]);
	  //for(var i = 0; i < target.length; i++) {
		  //if(target[i] === $gamePlayer) {
			  $gameScreen.startShake(power,speed,duration);
  ...
  Skill_Sequencer.prototype.startAccel = function(action) {
	    this._skill.accel = Number(action[1]) / Number(action[2]);
	    this._skill.accelD = Number(action[2]);
  };
	  
  Skill_Sequencer.prototype.startRotate = function(action) {
	    this._skill.rotate = (2 * Math.PI * Number(action[1])) / (360 * Number(action[2]));
	    this._skill.rotateD = Number(action[2]);
  };	  
	  
  Skill_Sequencer.prototype.startGuid = function(action) {
        this._skill.guidSpeed = Number(action[1]) * Math.PI / 180 / Number(action[3]);
        this._skill.guidA = Number(action[2]) * Math.PI / 180 / Number(action[3]);
        this._skill.guidD = Number(action[3]);
  };	  
  ...
  Skill_Sequencer.prototype.towardPlayer = function() {
  ...
  Skill_Sequencer.prototype.targetJump = function(action, targets) {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      if(!targets[i].battler().states().contains($dataStates[11],$dataStates[12],$dataStates[17]) && !targets[i]._invincible && !targets[i]._avoid && !targets[i]._noKnock)
      dist ? targets[i].pixelJumpFixed(dir, dist) : targets[i].jumpFixed(dir);
    }
  };
  
  Skill_Sequencer.prototype.targetStar = function(action, targets) {
	    for (var i = 0; i < targets.length; i++) {

	        var x = this._skill.collider.center.x;
	        var y = this._skill.collider.center.y;
	        //QuasiABS.Manager.addPicture
	    }
	  };
  ...
  Skill_Sequencer.prototype.userMove = function(action) {
    ...
    var dir = this._character._diagonal || this._character._direction; //action[2] === "backward" ? 0 : 5;
  ...
  Skill_Sequencer.prototype.userKnockBack = function(action) {
	  $gamePlayer.fixedMoveBackward(action[2])
	  };
  ...
  Skill_Sequencer.prototype.userJump = function(action) {
    ...
    if (action[2] === "backward") {
     ...
     /* $gameMap.events().forEach(function(event) {
    	  ...
    	  var list = [];
    	  list.push({code: 230, indent: 0, parameters: [600 || 0]});
    	  list.push({code: 0, indent: 0, parameters: [0]});
    	  self.setup(list, event.eventId);
      });*/
    }
  ...
  Skill_Sequencer.prototype.userSlide = function(action) {
    ...
    var x2 = this._skill.collider.center.x - 24;
    var y2 = this._skill.collider.center.y - 34;
  ...
  Skill_Sequencer.prototype.userInvincible = function(action) {
	  this._character._invincible = action[2] ===  "on" ? true : false;
  };
	
  Skill_Sequencer.prototype.userAvoid = function(action) {
	  this._character._avoid = action[2] ===  "on" ? true : false;
  };
  ...
  Skill_Sequencer.prototype.userPoseMove = function(action) {
	  ...
	  var isAtking = this._character._pose.substring(0,3) === "btk";
	  var pose = action[2];
	  var wait = action[3] === "true";
	  if (isMoving) {
		  pose += "move";
		  if(!(this._character._pose === "btkmove"+dir)) {
	...
	Skill_Sequencer.prototype.userClearPose = function(action) {
		if(!TouchInput.isPressed())
		  this._character.clearPose();
		};
	  
  Skill_Sequencer.prototype.userForceSkill = function(action) {
	  var id = Number(action[2]);
	    var angleOffset = Number(action[3]) || 0;
	    var mode = Number(action[4]) || 0;
	    var rat = Number(action[5]) || 1;
	    var repeat = Number(action[5]) || 1;
	    var rad = Number(action[6]) || 0;
	    switch (mode) {
	    	case 0 : 
	    		if (angleOffset) {
    		      ...
    		      //this._character._radian = Math.atan2(-(y1 - y2), x1 - x2);
    		    ...
    		    if (angleOffset) {
	    		    this._character._radian -= radOffset;
	    		    this._character._radian -= this._character._radian >= (2 * Math.PI) ? 2 * Math.PI : 0;
	    		    }
    		    break;
	    	case 1 :
	    		this._character.radianTowardsPlayer();
	    	...
	    	case 2 :
	    	    ...
	    	    toRadian += toRadian < 0 ? 2 * Math.PI : 0;
	    	    var sat = Math.sin(this._character._radian - toRadian);
	    	    var sat2 = Math.cos(this._character._radian - toRadian);
	  	       if(sat > 0 && sat2 < 0.9995) {
	 	    	   this._character._radian -= 0.03 * rat;
	 	       } else if(sat <= 0 && sat2 < 0.9995) {
		 	    	  this._character._radian += 0.03 * rat;
	 	       }
	 	      if (angleOffset) {
	    	...
	    	case 4 :
	    		var radOffset = angleOffset * Math.PI / 180;
	    		var skillRad = this._skill.radian + radOffset;
	    		skillRad += skillRad < 0 ? 2 * Math.PI : 0;
	    		for(var i = 0; i < repeat; i++) {
	    		    this._character.forceSkill2(id, true, this._skill.collider._center.x, this._skill.collider._center.y, skillRad);
	    		    skillRad += rad * Math.PI / 180;
	    		}
	    		break;
	    	case 5 :
	    		var radOffset = angleOffset * Math.PI / 180;
	    		...
	    		skillRad += skillRad < 0 ? 2 * Math.PI : 0;
	    		for(var i = 0; i < repeat; i++) {
	    		    this._character.forceSkill2(id, true, this._character._realPX, this._character._realPY, skillRad);
	    		    this._skill.radian += rad * Math.PI / 180;
	    		}
	    		break;
	    	case 6 : 
	    		if (angleOffset) {
  ...
  Skill_Sequencer.prototype.userForceSkill2 = function(action) {
	    var id = Number(action[2]);
	  ...
	  Skill_Sequencer.prototype.userForceSkill3 = function(action) {
		    var id = Number(action[2]);
		    var angleOffset = Number(action[3]);
		    if (angleOffset) {
		      var radOffset = angleOffset * Math.PI / 180;
...
//변경 : 스킬 텔레포트 생성
  ...
  //변경 : 스킬 대쉬 생성
  Skill_Sequencer.prototype.skillDash = function(action) {
  ...
  Skill_Sequencer.prototype.startMove = function(action) {
    ...
    if (radian === "diagonal") {
    	if(this._character._diagonal) {
    		radian = this.directionToRadian(this._character._diagonal);
    		distance *= 1.4;
    	}
    	else {
    		radian = this.directionToRadian(this._character._direction);
    	}
    }
    this.setSkillRadian(Number(radian));
...
//변경 : 스킬 스프라이트(발사체)의 우선순위 22로 변경(기존 3) , 파일이름에 y축 변수 추가
  Skill_Sequencer.prototype.showPicture = function(action) {
    ...
    if (animated) {
      ...
      this._skill.picture._layers = Math.floor(Number(settings[0])/5) || 1;
      this._skill.picture._speed = Number(settings[1]) || 1;
    ...
    this._skill.picture.rotate = Number(action[4]) || 0;
    var anchorX = action[5] || 0.5;
    var anchorY = action[6] || 0.5;
    this._skill.picture.z = Number(action[7]) || 3;//22;
    this._skill.picture.anchor.x = anchorX;
    this._skill.picture.anchor.y = anchorY;
    this.setSkillPictureRadian(this._skill.picture, this._skill.radian);
    //SceneManager.particleGens[0].emit("Spark")
...
//스킬이 맞았는지 충돌검사
  Skill_Sequencer.prototype.canSkillMove = function() {
    ...
    if (targets.length > 0) {
      for (var j = targets.length - 1; j >= 0; j--) {
    	  if(targets[j]._avoid) {
    		  if(targets[j].battler().weapons().length) {
    			  targets[j].battler().addState(15);
    		  }
    	  }
        if (!this._skill.targetsHit.contains(targets[j].battler()._charaId) && !targets[j]._invincible && !targets[j]._avoid) {
    ...
    if (this._skill.settings.through === 2 || this._skill.settings.through === 3) {
    	var color = QuasiMovement._collisionmap.getColor(this._skill.collider.x, this._skill.collider.y);
		switch(color) {
		case "#ff0000" :
			this.stop = true;
			break;
		case "#0000ff" :
			if(this._skill.settings.overwater){
				break;
			}
			else this.stop = true;
			break;
    ...
    */
    ...
    if (!$gameMap.isLoopHorizontal()) {
      if (x1 < 0 || x2 >= maxW) this.stop = true;
    }
    if (!$gameMap.isLoopVertical()) {
      if (y1 < 0 || y2 >= maxH) this.stop = true;
    }
    if (this.stop) {
  	  QuasiABS.Manager.startAnimation(this._skill.data.animationId, this._skill.collider.center.x, this._skill.collider.center.y);
      this._skill.targetsHit = [];
      this._skill.moving = false;
      this._skill.duration = 0;
      this._waitForMove = false;
      return false;
    }
    return true;
  ...
  Game_Map.prototype.setup = function(mapId) {
	    $gamePlayer._agroList  = {}; // List of charas that attacked this
	    $gamePlayer._agrodList = []; // List of charas that this attacked
	    $gamePlayer._inCombat = false;
	    $gamePlayer._casting = null;
	    $gamePlayer._skillLocked = [];
	    $gamePlayer._invincible = false;
	    $gamePlayer._avoid = false;
  ...
  //enableEnemy 변경시 이부분 if 지울필요 있음
  ...
  Game_System.prototype.loadClassABSKeys = function() {
    ...
    if (classKeys && classKeys[1].trim() !== "") {
      var newKeys = QuasiABS.quickSlot[$gameParty.leader()._actorId - 1];	//QuasiABS.stringToSkillKeyObj(classKeys[1]);
...
//변경 : 
  ...
  Game_Battler.prototype.removeState = function(stateId) {
    if (this.isStateAffected(stateId)) {
      ...
      if ($dataStates[stateId].meta.bound) {
    	  this._bound = false;
    	  this._isBounded--;
      }
      if ($dataStates[stateId].meta.avoid) {
    	var weaponId = this.weapons()[0].baseItemId;
    	var oldSkill = QuasiABS._weaponSkills[weaponId][12].skillId
      	QuasiABS._weaponSkills[weaponId][12].skillId = QuasiABS._weaponSkills[weaponId][1].skillId;
    	QuasiABS._weaponSkills[weaponId][1].skillId = oldSkill;
  	    $gameSystem.preloadSkills();
      }
    }
  ...
  Game_Battler.prototype.isBounded = function() {
	    return this._isBounded > 0
	  };
  ...
  Game_BattlerBase.prototype.addNewState = function(stateId) {
    ...
    if ($dataStates[stateId].meta.avoid) {
    	var weaponId = this.weapons()[0].baseItemId;
    	var avoidSkill = QuasiABS._weaponSkills[weaponId][12].skillId
    	QuasiABS._weaponSkills[weaponId][12].skillId = QuasiABS._weaponSkills[weaponId][1].skillId;
    	QuasiABS._weaponSkills[weaponId][1].skillId = avoidSkill;
    	$gameSystem.preloadSkills();
    }
  };
  ...
  //변경 : initWeaponSkills 에서 방어구도 추가
  Game_Actor.prototype.initEquipSkills = function() {
    if (this === $gameParty.leader()) {
      for (var i = 0; i < this._equips.length; i++) {
        if (this._equips[i].object()) {
          ...
          //방어구일 때
          console.log("초기화때 방어구를 입습니다.");
          if (this._equips[i].isArmor() && equipId) {
              this.changeArmorEffect(equipId, true);
            }
        }
      }
    }
 ...
 //변경 : 체인지이큅 방어구도 액션취함
  ...
  Game_Actor.prototype.changeEquip = function(slotId, item) {
    ...
    if (equipId !== oldId && this._equips[slotId].isArmor()) {
        console.log("방어구를 변경하셨네요. 효과를 적용합니다", equipId, oldId);
    	this.changeArmorEffect(equipId, true);
    ...
    if (equipId === 0) {
    	...
    	else if(oldId) this.changeArmorEffect(oldId, false);
    }
    $stvArmorSets.check();
  };
  ...
  Game_Actor.prototype.changeArmorEffect = function(id, equip) {
	...
	if(effect) {
		...
		for (var i = 0; i < effect.length; i++) {
			if (effect[i] !== "") {
			actions = QuasiMovement.stringToAry(effect[i], true);
			action = actions.split(' ');
			this.ArmorEffect(action, equip);
			}
		}
	}
  };
  ...
  Game_Actor.prototype.ArmorEffect = function(action, equip) {
	  console.log("아머이펙트 작동", action);
	switch(action[0].toLowerCase()) {
	case "movespeed" :
		var amt = Number(action[1]) || 0;
		console.log("이동속도가 증가할것입니다. 이만큼", amt, equip);
		this._moveSpeed = equip? this._moveSpeed + amt : this._moveSpeed -= amt; 	
		break;
	}
  };
...
//변경 : 넉백 추가, 섀터 추가, noMove추가
  ...
  Game_Enemy.prototype.setup = function(enemyId, x, y) {
    ...
    var team = /<team:([0-9]*?)>/i.exec(notes);
    this._team  = team? Number(team[1]) : 2;
    var approach = /<approach:([0-9]*?)>/i.exec(notes);
    this._approach = approach? Number(approach[1]) : 0;
    this._noKnock = /<noknock>/i.test(notes);
    this._noShatter = /<noshatter>/i.test(notes);
    this._noMove = /<nomove>/i.test(notes);
    this._bound = false;
    this._loot = [];
  };
...
//변경 : 스킬 쿨다운셋 액터에게 주기, 스킬 차지 추가
  Game_CharacterBase.prototype.updateSkills = function() {
    ...
    /*switch($gameParty.leader().actorId()){
  	  case 1 :
  		  if(cooldownsFor[0])
  		  $gamePlayer._skillCooldowns = cooldownsFor[0]; 
  		  //changeActor = false;
  		  break;
  	  case 2 :
  		if(cooldownsFor[1])
  		$gamePlayer._skillCooldowns = cooldownsFor[1];
  		//changeActor = false;
  		  break;
  	  case 3 :
  		if(cooldownsFor[2])
  		$gamePlayer._skillCooldowns = cooldownsFor[2];
  		//changeActor = false;
  		  break;
    	}*/
    if(cooldownsFor[0])
  ...
  Game_CharacterBase.prototype.forceSkill = function(skillId, forced) {
    if (!this.battler() || this._invincible || this._avoid) return;
    var skill = {};
  ...
  Game_CharacterBase.prototype.forceSkill2 = function(skillId, forced, colx, coly, rad) {
	    ...
	    skill.ondmg    = QuasiABS.getSkillOnDamage(skill.data);
	    if (!skill.settings || skill.data.scope === 0) return;
	    skill.collider  = this.makeSkillCollider2(skill.settings, colx, coly);
	    skill.sequencer = new Skill_Sequencer(this, skill);
	    skill.userDirection = this._direction;
	    skill.radian = rad || this.directionToRadian(this._direction);
	    //this._radian = null;
	    skill.direction = this._direction;
	    skill.targetsHit = [];
	    skill.forced = forced;
  ...
  Game_CharacterBase.prototype.makeTargetingSkill = function(skill) {
    ...
    skill.picture = new Sprite();
    skill.picture.bitmap = ImageManager.loadPicture(skill.settings.groundimage);
    if (this._selecttargeting) {
      ...
      skill.picture.rotatable = "true";
      skill.picture.originDirection = 2;
      skill.picture.z = 1;//22;
      skill.picture.anchor.x = 0.5;
      skill.picture.anchor.y = 0.5;
      skill.picture.move(x, y);
      skill.picture.opacity = 240;
    }
  ...
  Game_CharacterBase.prototype.makeSkillCollider = function(settings) {
    ...
    var infront  = settings.infront === true;
    var rotate   = settings.rotate === true;
    var offset   = Number(settings.offset);
    ...
    var x1 = this.cx() - collider.center.x + Math.cos(rad) * offset;
    var y1 = this.cy() - collider.center.y - Math.sin(rad) * offset;
    if (infront) {
  ...
  Game_CharacterBase.prototype.makeSkillCollider2 = function(settings, colx, coly) {
	    settings.collider = settings.collider || ["box", this.collider().width, this.collider().height];
	    var collider, w, h;
	    var width    =  w = settings.collider[1];
	    var height   =  h = settings.collider[2];
	    ...
	    var x1 = colx - collider.center.x + Math.cos(rad) * offset;
	    var y1 = coly - collider.center.y - Math.sin(rad) * offset;
	    if (infront) {
	      var w1 = this.collider().width;
	      var w2 = collider.width;
	      var h1 = this.collider().height;
  ...
  Game_Player.prototype.canMove = function() {
	  if (this.battler()._bound) return false;
    if (this.battler() && this._skillLocked.length > 0) return false;
...
//변경 : 커먼이벤트 발동하게
  ...
  Game_Player.prototype.updateABS = function() {
    ...
    /*if($gamePlayer.actor().tp < $gamePlayer.actor().maxTp()) {
    	switch($gamePlayer.actor().actorId()){
    		case 1:
    			$gamePlayer.actor()._tp += 0.1;
    			break;
    		case 2:
    			$gamePlayer.actor()._tp += 0.2;
    			break;
    		case 3:	
    			$gamePlayer.actor()._tp += 0.15;
    			break;
  ...
  Game_Player.prototype.updateGroundTargetingPosition = function() {
    ...
    //skill.collider.color = skill.isOk ? 0x33ffff : 0xff0000;
    skill.picture.rotation += 0.005;
  };
  ...
  Game_Player.prototype.updateInput = function() {
	  var absKeys = $gameSystem.absKeys();
	    ...
	    for (var key in absKeys) {
	      ...
	      //var soulOn = Input.isPressed('shift');
	     // if(soulOn) {
	    //	  $gameTemp.reserveCommonEvent(6 - soulOn);
	     // }
	     // else {
	    //	  $gameTemp.reserveCommonEvent(6);
	      if (Input.isTriggered(input)) {      
	    	  //else
	    		//  $gameTemp.reserveCommonEvent(6);
	    	  /*if(Input.isTriggered('shift') && $gamePlayer.actor() === $gameActors.actor(1))
	    		  {
	    ...
	    //	  }
	      ...
	      if (input === "mouse1" && TouchInput.isPressed() && !Input.isPressed('control') && this.canClick() && !$gameSwitches.value(10)) {
	        TouchInput._triggered = false;
	        //if($gamePlayer)
	        this.useSkill(absKeys[key].skillId);
	      }
	      if (input === "mouse2" && TouchInput.isCancelled() && !Input.isPressed('control') &&this.canClick() && !$gameSwitches.value(10)) {
...
//변경 : 흩날, 넉백 추가
  Game_Event.prototype.setupBattler = function() {
    ...
    if (foe && !disabled) {
      ...
      this._noShatter = this._battler._noShatter;
      this._team = this._battler._team;
      this._approach = this._battler._approach;
  ...
  Game_Event.prototype.distToPlayer = function() {
	  return Math.sqrt(Math.pow($gamePlayer.x - $gameMap.event(this._eventId).x,2) + 
	  		 Math.pow($gamePlayer.y - $gameMap.event(this._eventId).y,2));
  }
  ...
  Game_Event.prototype.enableEnemy = function() {
	    $gameSystem.enableEnemy(this._mapId, this._eventId);
	    this.clearABS();
	    this._battler = null;
	  };
...
//변경 : 차지 있으면 유즈에이블 못함
  ...
  Game_Event.prototype.updateAI = function() {
    ...
    if (this._aiWait1 >= QuasiABS.aiWait1) {
      ...
      this._aiWait1 = 0;
      this._toDistance = Math.sqrt(Math.pow($gamePlayer.x - $gameMap.event(this._eventId).x,2)+Math.pow($gamePlayer.y - $gameMap.event(this._eventId).y,2));
    } else {
      this._aiWait1++;
    }
    //var distance = Math.sqrt(Math.pow($gamePlayer.x - $gameMap.event(this._eventId).x,2)+Math.pow($gamePlayer.y - $gameMap.event(this._eventId).y,2));
    if (bestAction) {
      this.useSkill(bestAction);
    } else if (this.canMove() && !this._noMove && !this.battler()._bound && $gameSelfSwitches.value(key) == true) {
      if (Imported.Quasi_PathFind && QuasiABS.aiAStar) {
        ...
        if (this._freqCount < this.freqThreshold()) {
        	if(this._approach > 0) {
        		if(this._toDistance > this._approach + 4) {
        			this.moveTowardCharacter(bestTarget);
        		} else if (this._toDistance < this._approach){
        			this.moveAwayFromCharacter(bestTarget);
        		}
        	} else {
        		this.moveTowardCharacter(bestTarget);
        	}
			} else if (this._toDistance < this._approach - 4){
				this.moveAwayFromCharacter(bestTarget);
...
//변경 : 전투끝나면 그자리 멈춤
  ...
  Game_Event.prototype.onDeath = function() {
	    if (this._onDeath) {
	    	...
	    	var oDt = this._onDeath;
	    	//this._shatter = [true,[],true,1,1,1];
	        $gameSelfSwitches.setValue(key, true);
	        //if (!this._noShatter)
	        	//this._shatter = [true,[],true,1,1,1];
	    ...
	    this._loot = this.battler().makeDropItems();
	    this._battler = null;
	    if (!this._dontErase) {
	      this.erase();
	    }
...
//변경 :  interp, args, 커맨드 추가
  var $gameInterp = new Game_Interpreter();
    Game_Loot.prototype.setItem = function(item) {
      this._loot = item;
      this.setIcon(item.iconIndex);
      /*args =('particleSet '+this._eventId+' above Test test').split(" ");
      command = args.shift();
      $gameInterp.pluginCommand(command, args);
      args =('particleOn '+this._eventId+' Test').split(" ");
      command = args.shift();
      $gameInterp.pluginCommand(command, args);*/
      args =('particleSet '+this._eventId+' below Item_rare particle').split(" ");
      command = args.shift();
      $gameInterp.pluginCommand(command, args);
      var args =('particleOn '+this._eventId+' Item_rare').split(" ");
  ...
  Game_Loot.prototype.setIcon = function(iconIndex) {
    ...
    this._itemIcon.z = 1;
    this._itemIcon.transform.scale._x = 0.7;
    this._itemIcon.transform.scale._y = 0.7;
...
//변경 : 리무브 지움
  Game_Loot.prototype.collectDrops = function() {
    ...
    var args =('particleOff '+this._eventId+' Item_rare').split(" ");
    var command = args.shift();
    $gameInterp.pluginCommand(command, args);
  };
...
//변경 : 180프레임 전 아이템 없어질때 이팩트도
  Game_Loot.prototype.update = function() {
    ...
    if (this._decay === 180) {
    	this.erase();
        var args =('particleOff '+this._eventId+' Fire').split(" ");
        var command = args.shift();
    	$gameInterp.pluginCommand(command, args);
    	QuasiABS.Manager.removePicture(this._itemIcon);
    }
...
//변경 : 더월드 상태일때 프레임 안움직임
  ...
  AnimatedSprite.prototype.setFramePosition = function() {
    var pw = this._bitmap.width / 5;
    var ph = this._bitmap.height / this._layers;
    var sx = pw * (this._pattern % 5);
    var sy = ph * Math.floor(this._pattern / 5)
  ...
  Sprite_Character.prototype.updateDamagePopup = function() {
    ...
    if (this._damages.length > 0) {
      for (var i = 0; i < this._damages.length; i++) {
        ...
        this._damages[i].y -= 30 + $gameMap.displayY() * $gameMap.tileHeight();
...
//변경 : 데미지팝업 방식변경
  Sprite_Character.prototype.setupDamagePopup = function() {
    ...
    if (this._battler._damageQueue.length > 0) {
      ...
      sprite.y = sprite.realY = this._character.cy() - this.height/2 + 20;
      }
      var string;
      var style = {};
      ...
      if (result.missed || result.evaded) {
        ...
        if (dmg >= 0) {
          string = dmg;
          style.fill = isPlayer? "#FF8C00" : "#ffffff";
        } else {
          string = Math.abs(dmg);
          style.fill = "#00ff00";
        }
      ...
      if (result.critical) {
    	  ...
    	  else style.fill = "#ff007e";
      }
      settings = {};
      var fadeout = "20 30 fadeout"; //페이드아웃 시작시간, 감소속도 역속도임
      var slideup = "0 120 slideup 80"; // 모르겠음, 속도?
      settings.transitions = [fadeout, slideup];
  ...
  Spriteset_Map.prototype.updatePictures = function() {
    ...
    for (var i = 0; i < this._pictures.length; i++) {
      ...
      if(this._pictures[i].rotate > 0) this._pictures[i].rotation += this._pictures[i].rotate;
    }
  };
