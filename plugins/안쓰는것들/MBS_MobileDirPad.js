...
(function ($) {
	...
	Sprite_Button.prototype.updateTouch = function() {
		if (this._type == 0 && TouchInput.isPressed()) {
			...
			Input._currentState['dash'] = rect.contains(TouchInput.x, TouchInput.y);
		} else if (this._type == 0) {
			Input._currentState['dash'] = false;
		} else if (this._type == 1 && TouchInput.isTriggered()) {
			var rect = new PIXI.Rectangle(this.x - this.width * this.anchor.x, this.y - this.height * this.anchor.y, this.width, this.height);
			Input._currentState['jump'] = rect.contains(TouchInput.x, TouchInput.y);
		} else if (this._type == 1) {
			Input._currentState['jump'] = false;
		}
	};
...
TouchInput.isNotPad = function() {
	 var x = TouchInput.x;
	 var y = TouchInput.y;
	if(x<76 || x>180 || y<400 || y>504)
