...
(function() {
  ...
  //-----------------------------------------------------------------------------
  // Game_Player 설정할 경우 링커맨드시 플레이어 이동 불가능
  //
  /*
  ...
  */
  ...
  Sprite_TMRingCommandCaption.prototype.refresh = function() {
    ...
    //this.bitmap.fillRoundRect(0, 0, 240, 48, 30);
...
//커맨드 없앰 포메이션, 세이브
    ...
    if ($gameTemp.isRingCommandVisible()) {
      if (this._commandRadius < 4) {
        ...
        if (openGoldWindow && this.parent.parent) {
          ...
          goldWindow.setBackgroundType(2);
          goldWindow.open();
        }
  ...
  Sprite_TMRingCommand.prototype.updateInput = function() {
    ...
    if (TouchInput.isCancelled() || TouchInput._wheelY > 0) {
      this._commandIndex++;
