...
(function() {
  ...
  QuasiABS.getSkillSettings = function(skill) {
    if (!this._skillSettings.hasOwnProperty(skill.id)) {
      ...
      if (settings) {
        ...
        this._skillSettings[skill.id].charge = Number(settings.charge) || 0;
        this._skillSettings[skill.id].charging = Number(settings.charging) || 0;
      }
    }
  ...
  QuasiABS.Manager.startAction = function(self, targets, item) {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      if(item.settings.jumping + targets[i].battler().states().contains($dataStates[12]) < 2 && !targets[i].battler().states().contains($dataStates[11]))
...
//변경 : globalLock에 메인캐스팅 변수 추가, 텔레포트 추가
  ...
  Skill_Sequencer.prototype.userForceSkill = function(action) {
	  var id = Number(action[2]);
	    ...
	    if (angleOffset) {
	       ...
	       var x2 = this._character.cx(); //this.cx();
	       var y2 = this._character.cy(); //cy();
	       // this._radian = Math.atan2(-(y1 - y2), x1 - x2);
	       // this._radian += this._radian < 0 ? 2 * Math.PI : 0;
	    	//this._character._radian = this.directionTowards(x, y);
  ...
  Skill_Sequencer.prototype.shiftTeleport = function(action) {
...
//변경 : 스킬 쿨다운셋 액터에게 주기, 스킬 차지 추가
  Game_CharacterBase.prototype.updateSkills = function() {
    ...
    for (var id in this._skillCharges) {
        if (this._skillCharges.hasOwnProperty(id)) {
        	skill = $dataSkills[id];
        	skill.settings = QuasiABS.getSkillSettings(skill);
        	if (this._skillCharges[id] > skill.settings.charge) {
        		delete this._skillCharges[id];
            } else {
            	this._skillCharges[id] += skill.settings.charging;
            	//alert(this._skillCharges[id]);
          };
        }
      }
  ...
  Game_CharacterBase.prototype.onTargetingEnd = function() {
    ...
    this._skillCharges[skill.data.id] = skill.settings.charge - 60;
  ...
  Game_CharacterBase.prototype.forceSkill = function(skillId, forced) {
    ...
    if(this._skillCharges[skillId] < skill.settings.charge) {
    	this._skillCharges[skillId] -= 60;
    } else this._skillCharges[skillId] = skill.settings.charge - 60;
...
//차지 있으면 유즈에이블 못함
  Game_Player.prototype.usableSkills = function() {
    return this.battler().skills().filter(function(skill) {
      ...
      if (this._skillCharges[skill.id] < 60) return;
      return true;
...
//변경 : 커먼이벤트 발동하게
  ...
  Game_Player.prototype.updateGroundTargetingPosition = function() {
    ...
    skill.collider.color = skill.isOk ? 0xffffff : 0xff0000;
  };
  ...
  Game_Player.prototype.updateInput = function() {
	  var absKeys = $gameSystem.absKeys();
	    ...
	    //	  }
	      ...
	      if (input === "mouse1" && TouchInput.isTriggered() && !Input.isPressed('control') && this.canClick()&!$gameSwitches.value(10)) {
	        TouchInput._triggered = false;
	        //if($gamePlayer)
	        this.useSkill(absKeys[key].skillId);
	      }
	      if (input === "mouse2" && TouchInput.isPressed2() && !Input.isPressed('control') &&this.canClick()&!$gameSwitches.value(10)) {
...
//변경 : 차지 있으면 유즈에이블 못함
  Game_Event.prototype.usableSkills = function() {
    ...
    return this._skillList.filter(function(skillId) {
      ...
      if (this._skillCharges[skillId] < 60) return false;
      return true;
    }, this);
  };
  ...
  Game_Event.prototype.updateAI = function() {
    ...
    } else if (this.canMove() && !this._noMove && $gameSelfSwitches.value(key) !== true) {
...
//변경 :  interp, args, 커맨드 추가
  var $gameInterp = new Game_Interpreter();
    Game_Loot.prototype.setItem = function(item) {
      ...
      console.log(this);
      args =('particleSet '+this._eventId+' above Fire flame').split(" ");
      command = args.shift();
      $gameInterp.pluginCommand(command, args);
      args =('particleOn '+this._eventId+' Fire').split(" ");
      command = args.shift();
      $gameInterp.pluginCommand(command, args);
    };
