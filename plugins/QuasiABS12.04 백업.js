  ...
  Skill_Sequencer.prototype.updateSkillPosition = function() {
    ...
    if(this._skill.accelD || this._skill.rotateD || this._skill.guidD){
    	...
    	if(this._skill.guidD) {
    		if(x1 < $gamePlayer.px) {
    	    	this._skill.speedX += this._skill.guidSpeed;
    	    } else this._skill.speedX -= this._skill.guidSpeed;
    	    if(y1 < $gamePlayer.py ) {
    	    	this._skill.speedY += this._skill.guidSpeed;
    	    } else this._skill.speedY -= this._skill.guidSpeed;
    	}
...
//변경 : globalLock에 메인캐스팅 변수 추가, 텔레포트 추가, 지진 추가
  ...
  Skill_Sequencer.prototype.startGuid = function(action) {
        this._skill.guidSpeed = Number(action[1]);
	  ...
	  var id = Number(action[2]);
	    ...
	    switch (mode) {
	    	...
	    	case 2 :
	    	    ...
	    	    var sat = this._character._radian - toRadian;
	    	    sat += sat < 0 ? 2 * Math.PI : 0;
	 	       if(0.025 < sat && sat < Math.PI) {
	 	    	   this._character._radian -= 0.05 * rat;
	 	       } else if(Math.PI < sat && sat < 2 * Math.PI - 0.025) {
	 	    	  this._character._radian += 0.05 * rat;
	 	       }
	 	      if (angleOffset) {
	 		      var radOffset = angleOffset * Math.PI / 180;
    ...
    if (bestAction) {
      ...
      if (Imported.Quasi_PathFind && QuasiABS.aiAStar) {
        ...
        if (this._freqCount < this.freqThreshold()) {
        	if(this._approach > 0) {
        		if(distance > this._approach + 4) {
        			this.moveTowardCharacter(bestTarget);
        		} else if (distance < this._approach){
        			this.moveAwayFromCharacter(bestTarget);
        		}
        	} else {
        		this.moveTowardCharacter(bestTarget);
        	}
			//} else if (distance < this._approach - 4){
			//	this.moveAwayFromCharacter(bestTarget);
