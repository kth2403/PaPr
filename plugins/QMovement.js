...
(function() {
  ...
  Game_CharacterBase.prototype.pixelJump = function(xPlus, yPlus) {
    ...
    Game_CharacterBase.prototype.pixelZJump = function (xPlus, yPlus, zPlus) {
        xPlus /= QMovement.tileSize;
  ...
  Game_CharacterBase.prototype.pixelJumpForward = function(dist, dir) {
    ...
    this.jump(x, y);*/
		if(!dir)
			dir = this._diagonal? this._diagonal : this._direction;
	    dist = dist / QMovement.tileSize;
	    var x = dir === 6 ? dist : dir === 4 ? -dist : dir === 1 || dir === 7 ? -dist*0.7 : dir === 3 || dir === 9 ? dist*0.7 : 0;
	    var y = dir === 2 ? dist : dir === 8 ? -dist : dir === 7 || dir === 9 ? -dist*0.7 : dir === 1 || dir === 3 ? dist*0.7 : 0;
	    this.jump(x, y);  
  };

  Game_CharacterBase.prototype.spaceJump = function(dist) {
	    dir = Input.dir8;
	    dist = (dist / QMovement.tileSize) * (this.realMoveSpeed() - 1.5);
	    var x = dir === 6 ? dist : dir === 4 ? -dist : dir === 1 || dir === 7 ? -dist*0.7 : dir === 3 || dir === 9 ? dist*0.7 : 0;
	    var y = dir === 2 ? dist : dir === 8 ? -dist : dir === 7 || dir === 9 ? -dist*0.7 : dir === 1 || dir === 3 ? dist*0.7 : 0;
	    if (TMPlugin.CloudDust.JumpDusts > 0) {
	    	for (var i = 0; i < TMPlugin.CloudDust.JumpDusts; i++) {
    ...
    Game_CharacterBase.prototype.canJump = function (xPlus, yPlus) {
        ...
        var x2 = this.px + (xPlus * QMovement.tileSize);
        var y2 = this.py + (yPlus * QMovement.tileSize);
        ...
        this._findingJump = false;
        x2 = (x2 - this.px) / QMovement.tileSize;
        y2 = (y2 - this.py) / QMovement.tileSize;
        console.log(x2,y2)
        return [x2, y2];
    };
...
(function() {
  ...
  Game_Player.prototype.updateDashing = function() {
    ...
    //오류가 생길 경우 조건문을 event.constructor === Game_Event || event.constructor 넣어서 표현할것
    Game_Player.prototype.collidesWithEvent = function (event, type) {
      if (event._eventId > 0 && !event._erased) {
