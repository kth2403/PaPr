...
Game_Player.prototype.updateScroll = function(lastScrolledX, lastScrolledY) {
	...
	if(Game_Player.prototype._Map_Scroll_Enabled && Game_Player.prototype.Map_Scroll_for_this_Map){
		...
		if (x2 > xScroll-1) {$gameMap.startScroll(6,17,6);};//Left
		if (x2 < -0.5) {$gameMap.startScroll(4,17,6);};//Right
		if (y2 > yScroll-1) {$gameMap.startScroll(2,9,6);};//Down
		if (y2 < -0.5) {$gameMap.startScroll(8,9,6);};//Up	
