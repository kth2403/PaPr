...
( function() {
	...
	*/
	...
	Minimap_Display.prototype.initialize = function(){
		...
		this.mask.x = Math.min(Math.max($gameSystem.mmData.customLoc.x, 0), Graphics.boxWidth-($dataMap.width*$gameSystem.mmData.scale));
		this.mask.y =Math.min(Math.max($gameSystem.mmData.customLoc.y, 0), Graphics.boxHeight-($dataMap.height*$gameSystem.mmData.scale));
		this.mask.beginFill('#000000', 0);
		//this.mask.isMask = true;
	    this.mask.drawRect(0, 0, 40, 40);//$dataMap.width*($dataMap.meta.mmScale || $gameSystem.mmData.scale), $dataMap.width*$dataMap.height*($dataMap.meta.mmScale || $gameSystem.mmData.scale));
	    this.mask.endFill();
	...
	Minimap_Display.prototype.update = function(){
		...
		//this.mask.isMask = true;
		//this.mask.beginFill('#000000', 1);
	    //this.mask.drawRect($gamePlayer.x * ($dataMap.meta.mmScale || $gameSystem.mmData.scale) - 10, $gamePlayer.y * ($dataMap.meta.mmScale || $gameSystem.mmData.scale) - 10, 20, 20);//$dataMap.width*($dataMap.meta.mmScale || $gameSystem.mmData.scale), $dataMap.width*$dataMap.height*($dataMap.meta.mmScale || $gameSystem.mmData.scale));
	    //this.mask.endFill();
	...
	function Minimap_Fog() {this.initialize.apply(this); }
	Minimap_Fog.prototype = Object.create(Sprite_Base.prototype);
	Minimap_Fog.prototype.constructor = Minimap_Fog;
	
	Minimap_Fog.prototype.initialize = function() {
		Sprite_Base.prototype.initialize.call(this);
		this.x=Math.min(Math.max($gameSystem.mmData.customLoc.x, 0), Graphics.boxWidth-($dataMap.width*$gameSystem.mmData.scale));
		this.y=Math.min(Math.max($gameSystem.mmData.customLoc.y, 0), Graphics.boxHeight-($dataMap.height*$gameSystem.mmData.scale));
		this.opac = 1.0;
		/*this.fog = new PIXI.Graphics();
		this.fog.beginFill('#000000');
	    this.fog.drawRect(0, 0, $dataMap.width*($dataMap.meta.mmScale || $gameSystem.mmData.scale), $dataMap.width*($dataMap.height*($dataMap.meta.mmScale || $gameSystem.mmData.scale));
	    this.fog.endFill();
	    this.fog.cacheAsBitmap = true;*/
		//this.bitmap.fillRect(0, 0, this.bitmap.width, this.bitmap.height, "rgba(0, 0, 0, 1)");
		$gameSystem.mmData.plugins.fogInit.call(this);

	}
	
	Minimap_Fog.prototype.update = function() {
		this.bitmap.clearRect($gamePlayer.x * ($dataMap.meta.mmScale || $gameSystem.mmData.scale) - 10, $gamePlayer.y * ($dataMap.meta.mmScale || $gameSystem.mmData.scale)- 10, 20, 20)
		$gameSystem.mmData.plugins.fogUpdate.call(this);
	}
		...
		if($gameSystem.mmData.visualSettings!="Off")
		{
			if (mmItemCheck>0) {
				...
				if($gameSystem.mmData.visualSettings=="Medium" || $gameSystem.mmData.visualSettings=="High"){
					//this.FogMap = new Minimap_Fog();
					//this.addChild(this.FogMap);
