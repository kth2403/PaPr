...
Game_MapGenerator.prototype.initSymbolTable = function() {
    //기호 정의
    //  refXY      : 기호에 대응하는 타일의 트크 르의지도상의 좌표
...
Game_MapGenerator.prototype.generateMap = function() {
	// 변경 : minRoomSize, maxRoomSize
    this._minRoomSize = eval(MinRoomSize);
    this._minBlockSize = this._minRoomSize + 2;
    this._maxRoomSize = eval(MaxRoomSize);
    this._maxBlockSize = this._maxRoomSize + 2;
    // 변경 : MinRooms, MaxRooms
    if (eval(MinRooms) >= 2) {
        this._minRooms = eval(MinRooms);
    }
    else {
        this._minRooms = 2;
    }
    if (eval(MaxRooms) >= 2) {
        this._maxRooms = eval(MaxRooms);
    }
    else {
        this._maxRooms = 2;
    }
...
Game_MapGeneratorRoomAndPass.prototype.makeRooms = function() {
    ...
    this._blocks.forEach(function(block) {
        ...
        var room = {x:roomX, y:roomY, w:roomW, h:roomH, hasPass:{t:false, b:false, l:false, r:false}, index:roomNum};
        ...
        for(var direction in this._adjacentBlockIndexList[crntIndex]) {
            ...
            tgetIndexList.forEach(function(tgetIndex) {
                ...
                case 't':
                    ...
                    $gameMap.copyEventFrom(1, 163, crntPass.x, crntPass.y, true);
                    $gameMap.copyEventFrom(1, 164, tgetPass.x, tgetPass.y + tgetPass.h - 1, true);
                    console.log("t", crntPass.y, tgetPass.y, tgetPass.h);
                    break;
                case 'b':
                ...
                case 'l':
                    ...
                    console.log("l", crntPass.y, tgetPass.y, tgetPass.h);
                    break;
                case 'r':
