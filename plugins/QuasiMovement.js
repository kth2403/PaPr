...
(function() {
  ...
  Game_CharacterBase.prototype.spaceJump = function(dist) {
	    dir = Input.dir8;
	    dist = (dist / QuasiMovement.tileSize) * (this.realMoveSpeed() - 1.5);
	    var x = dir === 6 ? dist : dir === 4 ? -dist : dir === 1 || dir === 7 ? -dist*0.7 : dir === 3 || dir === 9 ? dist*0.7 : 0;
	    var y = dir === 2 ? dist : dir === 8 ? -dist : dir === 7 || dir === 9 ? -dist*0.7 : dir === 1 || dir === 3 ? dist*0.7 : 0;
...
//변경 : 팔로워
  ...
  Game_CharacterBase.prototype.fixedMoveBackward = function(dist) {
    var lastDirectionFix = this.isDirectionFixed();
    var dir = this._diagonal? this._diagonal : this._direction
    this.setDirectionFix(true);
    this.fixedMove(this.reverseDir(dir), dist);
    this.setDirectionFix(lastDirectionFix);
  };

  Game_CharacterBase.prototype.slideTo = function(x, y) {
    this._radian = Math.atan2(this._py - y, this._px - x);
    this._radian = this._radian < 0 ? this._radian + 2 * Math.PI : this._radian;
  ...
  Game_Character.prototype.radianToDirection = function(radian, useDiag) {
     {
      if (radian >= Math.PI / 8 && radian < 3 * Math.PI / 8) {
        ...
        return 7;
      } else if (radian >= 9 * Math.PI / 8 && radian < 11 * Math.PI / 8) {
        return 1;
      } else if (radian >= 13 * Math.PI / 8 && radian < 15 * Math.PI / 8) {
        return 3;
      }
    }
  ...
  Game_Player.prototype.canClick = function() {
    return false; // For ABS mainly, sets to false if you are over a window
  }
