...
/*:
 * @plugindesc (v1.1) Sistema de arremessar os eventos.
 * @author Moghunter
 *
...
Game_CharacterBase.prototype.canPassThrow = function(x, y, d) {
	...
	if (d === 1) {x3 = x-1; y3 = y + 1;
	} else if (d === 2) {x3 = x; y3 = y + 1;	
...
Game_Player.prototype.moveByInput = function() {
	...
	if (this._pickup.enabled && this.canMove()) {
	    if (TouchInput.isPressed()) {this.throwTarget();return};
		if (TouchInput.isCancelled()) {this.holdDirectionT();return};
    };
...
Game_Player.prototype.throwEvent = function(event) {
	var r = event._throw.range * 48;	var xr = 0;	var yr = 0;
	if(this._diagonal) {
		if (this._diagonal === 1) {
			x = this.px - 0.7*(r-1); y = this.py + 0.7*(r - 1); x2 = -0.7*r; y2 = 0.7*r;
			for (var i = 0; i < r; i++) {
				if (this.canPixelPass(x,y,5)) {xr = x2; yr = y2;break};
				x--;x2--;y++;y2++;
			};
		}
		else if (this._diagonal === 3) {
			x = this.px + 0.7*(r-1); y = this.py + 0.7*(r - 1); x2 = 0.7*r; y2 = 0.7*r;
			for (var i = 0; i < r; i++) {
		    	if (this.canPixelPass(x,y,5)) {xr = x2; yr = y2;break};
		    	x++;x2++;y++;y2++;
		...
		x = this.px; y = this.py + r - 1; x2 = 0; y2 = +r;
		for (var i = 0; i < r; i++) {
	    	if (this.canPixelPass(x,y,5)) {xr = x2; yr = y2;break};
			y--;y2--;
		};	
    } else if (this._direction === 4) {
		x = this.px - r + 1; y = this.py; x2 = -r; y2 = 0;
		for (var i = 0; i < r; i++) {
	    	if (this.canPixelPass(x,y,5)) {xr = x2; yr = y2;break};
			x++;x2++;
		};	    
	...
	console.log(xr,yr)
	//되면 - 던지고 안되
	if (Math.abs(xr) < 20 && Math.abs(yr) < 14) {return};
	if($gamePlayer.isJumping()) {return};
	event.jump(xr / 48,yr / 48)
