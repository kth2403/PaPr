...
(function() {
  ...
  QuasiABS.Manager.startAction = function(self, targets, item) {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      if(item.settings.jumping + targets[i].battler().states().contains($dataStates[12]) < 2 && !targets[i].battler().states().contains($dataStates[11]) && !targets[i]._invincible)
...
//변경 : globalLock에 메인캐스팅 변수 추가, 텔레포트 추가, 지진 추가
  ...
  Skill_Sequencer.prototype.targetJump = function(action, targets) {
    ...
    for (var i = 0; i < targets.length; i++) {
      ...
      if(!targets[i].battler().states().contains($dataStates[11]) && !targets[i].battler().states().contains($dataStates[12]) && !targets[i].battler().states().contains($dataStates[17]) && !targets[i]._noKnock)
  ...
  Skill_Sequencer.prototype.setCollider = function(action) {
    if (action[1] === "show") {
      if (!this._skill.pictureCollider) {
        ...
        console.log(this._skill.pictureCollider)
  ...
  Skill_Sequencer.prototype.canSkillMove = function() {
    ...
    if (targets.length > 0) {
      for (var j = targets.length - 1; j >= 0; j--) {
        if (!this._skill.targetsHit.contains(targets[j].battler()._charaId) && !targets[j]._invincible) {
