...
(function($) {
    ...
    */
    ...
    $._precacheCustomCursors = function() {
        if (StorageManager.isLocalMode() && Utils.isOptionValid('test')) {
            ...
            // Store in json
            fs.writeFileSync(path + this._indexFilename, JSON.stringify(files), 'utf8');
    ...
    */
    $._eventsXy = function (x, y) {
        return $gameMap.events().filter(function (event) {
            return Math.abs(x - event.cx()) < 20 && Math.abs(y - event.cy()) < 12;
        });
    }
    /**
    * Show custom cursor
    ...
    */
    TouchInput._checkForEventUnderMouse = function(pageX, pageY) {
        ...
        if ($.conditionsValidForMouseHoverCheck()) {
            var x = $gameMap.canvasToMapPX(Graphics.pageToCanvasX(pageX));
            var y = $gameMap.canvasToMapPY(Graphics.pageToCanvasY(pageY));
    ...
    */
    TouchInput._activateClickEvents = function(x, y) {
        ...
        if ($.conditionsValidForMouseHoverCheck()) {
            var x = $gameMap.canvasToMapPX(x);
            var y = $gameMap.canvasToMapPY(y);
