...
(function() {
  ...
  Game_CloudDust.prototype.setup = function(x, y, speed, radian) {
    ...
    this._count = 25;
    if (radian != null) {
      radian = +radian + Math.random() * 3 - 1.5;
    } else {
  ...
  Game_CharacterBase.prototype.addCloudDust = function(speed, radian) {
    $gameMap.addCloudDust(this._realX + 0.5, this._realY + 0.9, speed, radian);
  };
  ...
  if (TMPlugin.CloudDust.DashDusts > 0) {
    var _Game_Player_moveStraight = Game_Player.prototype.moveStraight;
    Game_Player.prototype.moveStraight = function(d) {
  	  var dir = this._direction;
      if (this.isDashing() && this.canPass(this.x, this.y, this._direction)) {
        if (dir === 2) {
          ...
          var radian = Math.PI * 5 / 4;
        } else if (dir === 3) {
          var radian = Math.PI * 7 / 4;
        } else if (dir === 7) {
          var radian = Math.PI * 3 / 4;
        } else if (dir === 9) {
          var radian = Math.PI / 4;
        }
        for (var i = 0; i < TMPlugin.CloudDust.DashDusts; i++) {
