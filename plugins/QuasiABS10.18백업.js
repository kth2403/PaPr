	  ...
	  var id = Number(action[2]);
	    ...
	    switch (mode) {
	    	...
	    	case 2 :
	 	       ...
	 	       if(this._character._radian - toRadian < -0.025*rat) {
	 	    	   this._character._radian += 0.05*rat;
	 	       } else if(this._character._radian - toRadian > 0.025*rat) {
	 	    	  this._character._radian -= 0.05*rat;
	 	       }
	 	      if (angleOffset) {
	 		      var radOffset = angleOffset * Math.PI / 180;
	    	...
	    	case 4 :
	    		...
	    		for(var i = 0; i < repeat; i++) {
	    		    this._character.forceSkill2(id, true, this._skill.collider.x, this._skill.collider.y, skillRad);
	    		    skillRad += rad;
	    		}
	    		break;
	    	default : 
...
//변경 : 스킬 스프라이트(발사체)의 우선순위 22로 변경(기존 3)
  Skill_Sequencer.prototype.showPicture = function(action) {
    ...
    this._skill.picture.z = Number(action[4]) || 2;//22;
...
//변경 : 넉백 추가, 섀터 추가, noMove추가
  ...
  Game_Enemy.prototype.setup = function(enemyId, x, y) {
    ...
    this._approach = approach? Number(approach[1]) : 4;
...
//변경 : 스킬 쿨다운셋 액터에게 주기, 스킬 차지 추가
  ...
  Game_CharacterBase.prototype.makeSkillCollider2 = function(settings, colx, coly) {
	    ...
	    var x1 = colx - collider.center.x;
	    var y1 = coly - collider.center.y;
	    if (infront) {
	      var w1 = this.collider().width;
	      var w2 = collider.width;
	      var h1 = this.collider().height;
...
//변경 : 차지 있으면 유즈에이블 못함
  ...
  Game_Event.prototype.updateAI = function() {
    ...
    var distance = Math.pow($gamePlayer.x - $gameMap.event(this._eventId).x,2)+Math.pow($gamePlayer.y - $gameMap.event(this._eventId).y,2);
    if (bestAction) {
